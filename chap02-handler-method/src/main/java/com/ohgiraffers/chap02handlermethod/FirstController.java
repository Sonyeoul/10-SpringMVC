...
//        String message = modifyName + " 메뉴 가격을" + modifyPrice + "원으로"+ "변경 했습니다";
//        System.out.println(message);
//        model.addAttribute("message",message);
//        return "first/messagePrinter";
//
//    }
    ...
    public String modifyMenu(Model model, @RequestParam Map<String,String> parameters){
        ...
        String message = modifyName + "메뉴 가격을" + modifyPrice + "원으로 변경함";
        System.out.println(message);
        model.addAttribute("message",message);
        return "first/messagePrinter";
    }
    ...
    * 클래스 레벨에 @SessionAttrubute 어노테이션을 이용하여 세션에 값을 담을 key 를 설정해두면,
    * model 영역에 해당하난 key로 값이 추가되는 경우 Session에 자동 등록한다
    * */
    @PostMapping("login2")
    ...
    //sessionStatus의 setCompleat() 메서드를 호출해야 사용이 만료된다
    @GetMapping("logout2")
